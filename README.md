...
The protocol-v3 Ubuntu helper reports a bounded, sanitized timeline for trusted download, install, restart, identity verification, health-check, timeout, and rollback boundaries. Open **Technical details** in the blocking update screen to see confirmed events, elapsed time, and the last host activity; it opens automatically after 60 seconds without confirmed activity or when recovery needs attention. Protocol-v2 remains compatible with the ordered stage checklist but has no event trace, while protocol-v1 shows one **Applying trusted update** row. Generic progress does not mean the transaction is stalled. Raw Docker output, host paths, commands, digests, exit codes, and updater logs are intentionally never displayed in the browser.

Existing Ubuntu installations must rerun a checksum-verified installer bundle to install or refresh the root-owned helper, systemd unit, and socket mount; that installer upgrades the deployment to the protocol-v3 helper. This preserves authentication data, Data Protection keys, source configuration, durable operation state, and every mounted source file. Old stuck update events cannot be reconstructed, so the first structured trace begins with the next update after the helper refresh. Updating the application image cannot replace the host helper. During Apply, only the ReachCommander application container is recreated; Docker Engine is never restarted. If an update ends in the administrator-attention state, use `sudo reachcommander update-log`, `sudo reachcommander update-log --follow`, `sudo journalctl -u reachcommander-updater.service --since today`, and `sudo reachcommander doctor` before retrying. System updates are intentionally unavailable for Windows development, macOS Docker Desktop, and manual container deployments. See the [Ubuntu installation guide](docs/deployment/ubuntu.md#automatic-system-update-control) for migration and recovery commands.
