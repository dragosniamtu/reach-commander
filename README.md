...
- **Lossless text conversion:** strict decoders and encoders preview legacy text, reject replacement-character loss, and preserve every original byte in collision-safe `_original` backups.
...
- **Host-safe preview transcoding:** FFmpeg work is serial, cancelable, limited to two codec threads with a low-CPU preset and reduced process priority, while installer-managed Docker deployments reserve CPU headroom for the host.
...
| Quality | 951 cross-platform .NET tests, 527 Angular unit tests, 2 PWA contract tests, and 75 Chromium browser scenarios |
...
- A contextual top toolbar for Multi-Rename, batch text Encoding, Add files, and active-panel search.
...
- Batch conversion for SRT, SUB, TXT, CSV, NFO, Markdown, and JSON with detected encodings, bounded text samples, tracked progress, and byte-exact backups.
...
An Ubuntu deployment created by the current checksum-verified installer can instead use **Add source** in the authenticated top toolbar. Enter a display name and one existing absolute Ubuntu host folder. The UI accepts a specific child directory such as `/srv/family-media`, not broad roots such as `/`, `/home`, `/srv`, or `/mnt`. Every ancestor must be a stable directory owned by root and not group- or world-writable; the source leaf may be runtime-owned and writable. A typical `/home/user/...` path therefore fails because the user-owned home directory is an ancestor. Use a narrow root-controlled stable mount below `/srv` rather than weakening or changing the home directory. Sources are read-only by default. Read/write requires an explicit read/write confirmation because it lets ReachCommander change or delete files in that host folder. Each source chip also has an × action that removes only the ReachCommander mapping after confirmation; it never deletes the host folder or its files, and the final remaining source cannot be removed.
...
Existing, older Ubuntu installations must rerun the latest checksum-verified installer once before managed source add/remove can work. An image-only update cannot replace the root-owned helper. Clean installations include the source helper, restricted `/run/reachcommander-updater` socket boundary, management command, and systemd service immediately; the application container still never mounts `/var/run/docker.sock`. See [Ubuntu: manage sources from the UI](docs/deployment/ubuntu.md#manage-sources-from-the-ui) for permission checks, CLI fallback, and support diagnostics.
...
The protocol-v3 Ubuntu helper reports a bounded, sanitized timeline for trusted download, install, restart, identity verification, health-check, timeout, and rollback boundaries. Open **Technical details** in the blocking update screen to see confirmed events, elapsed time, and the last host activity; it opens automatically after 60 seconds without confirmed activity or when recovery needs attention. **Download diagnostics** requests a protocol-v4 host health snapshot and saves a temporary five-file ZIP on your device without uploading it or retaining it on the server. The bundle contains only allowlisted statuses and the public trace; it excludes raw logs, credentials, tokens, paths, filenames, addresses, hostnames, environment values, image digests, container identifiers, and file contents. Protocol-v2 remains compatible with the ordered stage checklist but has no event trace, while protocol-v1 shows one **Applying trusted update** row. Generic progress does not mean the transaction is stalled.

Existing Ubuntu installations must rerun a checksum-verified installer bundle to install or refresh the root-owned helper, systemd unit, and socket mount; that installer upgrades the deployment to the protocol-v4 diagnostics helper while preserving protocol-v1 through v3 update compatibility. This preserves authentication data, Data Protection keys, source configuration, durable operation state, and every mounted source file. If the helper is older or unavailable, the browser still downloads a partial bundle that explains how to refresh it. Old stuck update events cannot be reconstructed, so the first structured trace begins with the next update after the helper refresh. Updating the application image cannot replace the host helper. During Apply, only the ReachCommander application container is recreated; Docker Engine is never restarted. If an update ends in the administrator-attention state, download diagnostics or run `sudo reachcommander support-bundle > reachcommander-support.zip`, then use the root-only `update-log`, journal, and doctor commands for deeper investigation. System updates are intentionally unavailable for Windows development, macOS Docker Desktop, and manual container deployments. See the [Ubuntu installation guide](docs/deployment/ubuntu.md#automatic-system-update-control) for migration and recovery commands.

## Active-panel toolbar and search

The toolbar on the left side of the top bar always reflects the active panel, source, and logical directory; hardware monitoring remains on the right. **Add source** configures one absolute Ubuntu host folder on a compatible installer-managed deployment, while the × on a source chip removes only that mapping after confirmation. Opening Multi-Rename, Encoding, or Add files captures the active panel context, so switching panels cannot redirect an operation already under review.
...
Source chips show `RO` for application read-only policy and `RW` for application write opt-in. Unavailable sources remain visible with an accessible explanation. F4 Rename, Multi-Rename, Encoding, and Add files require an available `RW` source, and the server still revalidates source policy, containment, symlinks, staleness, and actual storage permissions.
...
Select one or more supported text files in the same writable filesystem panel, then click **Encoding**. ReachCommander accepts `.srt`, `.sub`, `.txt`, `.csv`, `.nfo`, `.md`, and `.json` files. Review every detected source encoding and bounded text sample—especially low-confidence legacy rows—override the source encoding when necessary, then choose UTF-8, UTF-8 with BOM, UTF-16 LE, Windows-1250, or Windows-1252 as the output.

Each successful conversion first preserves the exact original bytes as `<name>_original.<ext>`, then `<name>_original (2).<ext>` through `(999)` without overwriting an existing backup. Conversion is sequential and tracked; cancellation occurs only between file transactions. Strict conversion refuses undecodable text, characters that the selected output cannot represent, binary-looking or NUL-containing content, symbolic links, read-only sources, files larger than 32 MiB, and batches larger than 100 files. Automatic legacy detection is intentionally conservative rather than infallible, so verify low-confidence previews before converting. These backups are recovery aids, not a substitute for normal source backups.
...
Press `Enter` or double-click an MP4, MKV, or AVI file in a filesystem panel to open the blocking **Synchronize subtitles** workspace. ReachCommander automatically loads a same-name SRT from the same directory when present; the subtitle dropdown lists every non-symlink SRT beside that video for immediate selection. SRT is the only subtitle format in this release. Browser-ready H.264/AAC MP4 uses authenticated byte-range playback. MKV, AVI, or an incompatible MP4 is transcoded to temporary HLS by one bounded FFmpeg worker. The UI distinguishes **Waiting for preview worker** from active conversion, and Close remains available to cancel queued or running work.

Temporary assets live under `/data/media-previews`. The browser keeps a heartbeat running even after the first HLS segment becomes playable because FFmpeg may still be converting the rest of the movie. Explicit close cancels the process tree immediately; if a page disappears before its close request reaches the API, queued or still-running work is canceled after two minutes without a heartbeat. Completed ready sessions retain the existing 20-minute inactivity expiry. Server logs include the opaque session ID, safe display filename, queue/start/readiness/completion/cancellation transitions, FFmpeg process ID, 30-second progress, bounded redacted failure diagnostics, and cleanup failures.

The timing control applies one constant offset from `-600000` through `+600000` milliseconds. Positive values display subtitles later and negative values earlier. Defaults limit an SRT to 4 MiB and 20,000 cues, queue at most eight previews, cap each transcode at 90 minutes and 8 GiB of temporary output, require a pending/running browser heartbeat within two minutes, and capture no more than 64 KiB of process diagnostics. These limits are configurable under `MediaPreview`, but increasing them also increases CPU, disk, and denial-of-service exposure.
...
POST /api/text-encodings/{planId}/execute
GET /api/text-encodings/operations/{operationId}
POST /api/text-encodings/operations/{operationId}/cancel
...
- Copy, Move, MkDir, managed Trash, Restore, permanent Delete, Empty Trash, F4 Rename, Multi-Rename, batch Encoding, Add files, and archive extraction are the current write paths. All require explicit `readOnly: false`, re-resolve logical paths beneath configured sources, reject symbolic-link targets, reserved internal names, and unsafe traversal, and serialize overlapping mutations. The archive worker never receives the destination root or final names.
