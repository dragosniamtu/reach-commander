...
Press `Enter` or double-click an MP4, MKV, or AVI file in a filesystem panel to open the blocking **Synchronize subtitles** workspace. ReachCommander automatically loads a same-name SRT from the same directory when present; the subtitle dropdown lists every non-symlink SRT beside that video for immediate selection. SRT is the only subtitle format in this release. Browser-ready H.264/AAC MP4 uses authenticated byte-range playback. MKV, AVI, or an incompatible MP4 is transcoded to temporary HLS by one bounded FFmpeg worker; temporary assets live under `/data/media-previews` and are removed when the workspace closes or its 20-minute session expires.
