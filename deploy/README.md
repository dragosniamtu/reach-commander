...
Detailed download, install, restart, health-check, and rollback stages require the protocol-v2 Ubuntu helper in the current installer bundle. A protocol-v1 helper remains compatible and can complete the same protected update transaction, but the UI shows **Applying trusted update** instead of inventing unconfirmed steps; generic progress is not evidence that the update is stalled. Raw host logs, Docker output, paths, and commands are intentionally excluded from the browser contract. Use `sudo reachcommander doctor` for a terminal update failure.

Refresh an existing installation by rerunning a checksum-verified installer bundle. The refresh replaces the root-owned helper and unit while preserving authentication data, Data Protection keys, source configuration, durable operation state, and mounted source contents. An application-container update cannot upgrade the root-owned helper, so future helper changes require another verified installer refresh.
